...
const Equipment = () => {
  ...
  const handleRent = async () => {
    if (selectedSize) {
      ...
      try {
        const response = await authAxios.post(`/rent`, {
          size: selectedSize,
        });
        ...
        {showPopup && (
          <div style={styles.popupOverlay}>
            <div style={styles.popup}>
              ...
              <p style={styles.noticeText}>암벽화 대여하신 분들은 모두 이번 주 금요일까지 암벽화 반납 부탁드립니다.</p>
              <div style={styles.popupContent}>
                {sizes.map((item) => (
                  <button 
                    ...
                    style={{
                      ...styles.sizeButton,
                      backgroundColor: selectedSize === item.size ? 'blue' : item.rentable <= 0 ? 'gray' : 'initial', // 선택된 버튼과 비활성화된 버튼 색상 변경
                      color: selectedSize === item.size ? 'white' : item.rentable <= 0 ? 'lightgray' : 'initial', // 선택된 버튼과 비활성화된 버튼 텍스트 색상 변경
                      cursor: item.rentable <= 0 ? 'not-allowed' : 'pointer' // 비활성화된 버튼 커서 스타일 변경
                    }} 
                    onClick={() => handleSizeSelect(item.size)}
                    disabled={item.rentable <= 0} // 대여 가능한 암벽화가 없을 경우 버튼 비활성화
                  >
                    {item.size}
                  </button>
                ))}
              ...
              <div style={styles.buttonContainer}>
                <button style={styles.rentButton} onClick={handleRent}>대여하기</button>
